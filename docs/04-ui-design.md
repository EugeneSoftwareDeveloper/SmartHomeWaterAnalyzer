...
- **Дельта к прошлому замеру** отдельной строкой под значением — стрелка и число
  либо «без изменений». Отдельной строкой, а не рядом со значением: «1250 ppm»
  плюс «без изменений» не помещаются в строку рядом с бейджем зоны на узком экране.
  Цвет — только при смене зоны качества (зелёный лучше, красный хуже); внутри одной
  зоны и в пределах погрешности прибора дельта остаётся серой. Подробности —
  в `quality/trend.dart`.
- **Бейдж зоны** в шапке: цветной круг + текст зоны на полупрозрачном фоне.
- **Цветная шкала** с подписями min/max под полосой.
- **Тап по карточке** открывает страницу справки по этому параметру (`/help`),
  а не разворачивает описание на месте.

Карточка — `StatelessWidget`: собственного состояния у неё нет, всё приходит
параметрами.
...
└─ Scaffold(AppBar + body + FAB)
   ...
   │     ├─ SummaryHeader
   │     ├─ строка «Сравнение с замером …» / «Первый замер в этом месте»
...
### Выбор адреса замера (PlacePickerField)

`lib/ui/widgets/place_picker.dart`. Поле «Где мерим» над карточками.

Причина выбора из списка, а не свободного ввода: набирать название перед каждым замером утомительно, а результат — разнобой («кухня», «Кухня », «кран кухня»), из-за которого история плохо фильтруется.

- **Поле показывает весь путь** — «Дача · Скважина», «Дом · Кухня · Фильтр», — а не одно имя источника: «Кран на кухне» есть и дома, и на даче, и без пути их не различить.
- Под полем, когда адрес подставился по координатам, — строка «определено по координатам, N м» с иконкой `my_location`. Расстояние хранится числом, а подпись собирается в `build`: иначе после смены языка на экране осталась бы фраза на прежнем.
- Само поле — `InputDecorator` в `InkWell`; по тапу открывает нижний лист.
- **Лист сгруппирован иерархией**: заголовок места (с городом, если задан, и значком привязки к координатам), под ним источники прямо на месте, затем комнаты со своими источниками с отступом. Внизу — «Без адреса». В шапке кнопка «Настроить» ведёт на экран «Места замеров».
- Результат листа — отдельный тип `PlaceSelection`, а не голый `int?`: «закрыл лист» (результат `null`) и «выбрал „Без адреса“» (`sourceId == null`) — разные действия, и первое не должно сбрасывать выбор.
- Выбранный источник хранится ссылкой в `AppSettings.currentSourceId` и подставляется во все сохранения, пока пользователь его не сменит. В сам замер при этом уходят **имена** трёх уровней.
- Источник помечается использованным (`markSourceUsed`) и при выборе, и при сохранении — недавние поднимаются в начало вместе со своим местом и комнатой. Сбой этой отметки выбор не отменяет.

### Места замеров (PlacesPage)

`lib/ui/places_page.dart`, маршрут `/places`. Вход — из настроек и из кнопки «Настроить» в листе выбора.

Отдельный экран, а не режим редактирования внутри листа: выбор перед замером должен оставаться быстрым, а правка каталога — занятие редкое, ради которого не жалко открыть страницу.

- Каждое место — карточка с раскрытым `ExpansionTile`: источники прямо на месте, затем комнаты со своими источниками. Подзаголовок говорит, привязано ли место к координатам: «привязано по N замерам» или «без привязки — не подставляется автоматически».
- Меню места: переименовать (имя и необязательный город), добавить комнату, добавить источник, привязать здесь, сбросить привязку, удалить. У комнаты и источника — свои меню.
- **Удаление всегда спрашивает подтверждение** и прямо говорит, что история не пострадает: замеры хранят имена, а не ссылки.
- «Привязать здесь» — единственный способ задать координаты месту, где ещё ни разу не сохраняли замер. Здесь, в отличие от автоподстановки, разрешение на геолокацию запрашивать можно: это осознанное действие. Неудачный фикс якорь не переписывает — иначе одна попытка в подвале стёрла бы накопленную привязку.
- Диалоги имени держат `TextEditingController` в собственном состоянии: `showDialog` завершает Future в момент `pop`, а диалог после этого ещё анимируется, и освобождение контроллера сразу после `await` было бы обращением к уничтоженному объекту.
...
`_MeasurementChart` — `ConsumerStatefulWidget`. Состояние — `_selectedKey: String` (по умолчанию `'ph'`) и фильтр адреса. В шапке `DropdownButton` со значениями из `WaterParameterCatalog.forProfile(profile, l10n).map((p) => p.shortLabel)`.

Под шапкой — лента `ChoiceChip` для фильтра по адресу: «Все места» и адреса, которые встречаются в истории (`placesInHistory`). Лента появляется, только когда адресов больше одного. Чипы подписаны коротким видом адреса — комната внутри места редко нужна для узнавания, а лента узкая. Точка попадает в график через `measurementIsAt`, который нормализует обе стороны сравнения: иначе метка с пробелом по краю давала бы чип, не совпадающий ни с одной точкой. Если выбранный адрес исчез из истории (удалили все его замеры), график молча показывает всё, а не пустоту.
...
`groupMeasurementsByDay(rows, l10n)` из `lib/history/grouping.dart` (см. [`05-state-and-storage.md`](./05-state-and-storage.md#группировка-по-дням)) возвращает `List<MeasurementDayGroup>`. UI разворачивает каждую группу в `_DayHeader` (с иконкой `event_outlined`) + N тайлов.
...
Пункт «Язык» стоит рядом с темой: и то и другое — оформление, а не поведение. Список открывается тем же нижним листом, что тема и профиль норм.

Первым пунктом идёт «По системе» — состояние по умолчанию, и вернуться к нему должно быть так же просто, как уйти. Сами языки подписаны на себе: «Русский», «English». Переводить их нельзя — человек, случайно включивший незнакомый язык, ищет в этом списке дорогу назад.

Смена языка применяется сразу, без перезапуска: `MaterialApp.locale` читает настройку через `ref.watch`.
