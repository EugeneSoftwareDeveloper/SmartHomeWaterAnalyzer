...
└─ Scaffold(AppBar + body + FAB)
   ...
   │     ├─ PlacePickerField (поле «Место замера» → bottom sheet каталога)
   │     ├─ SummaryHeader
...
### Выбор места замера (PlacePickerField)

`lib/ui/widgets/place_picker.dart`. Заменил прежнее свободное текстовое поле «Метка замера».

Причина замены: набирать название перед каждым замером утомительно, а результат — разнобой («кухня», «Кухня », «кран кухня»), из-за которого история плохо фильтруется. Выбор из списка убирает и то, и другое.

- Само поле — `InputDecorator` в `InkWell`, выглядит как обычное поле ввода, но по тапу открывает bottom sheet.
- В листе: поле «Новое место» с кнопкой добавления, список мест с радио-индикатором выбранного, удаление с undo, отдельный пункт «Без места».
- Выбранное место лежит в `AppSettings.currentLabel` и подставляется во все последующие сохранения, пока пользователь его не сменит.
- Место помечается использованным (`lastUsedAt`) и при выборе, и при сохранении замера — так недавние поднимаются в начало списка.

### Геометка замера (LocationCard)

`lib/ui/widgets/location_card.dart`. Показывается в `HistoryDetailPage` **только** у замеров с координатами — у остальных карточки просто нет, никаких «координаты недоступны».

- Координаты в формате `55.763118, 37.828288` + строка точности «±12 м», если платформа её сообщила.
- Кнопка «Копировать» кладёт координаты в буфер обмена.
- Кнопка «На карте» пробует ссылки по порядку: сначала `geo:`-интент (его подхватит Яндекс.Карты, Google Maps, OsmAnd, 2ГИС), затем `https://www.google.com/maps/...` в браузере. Если не сработало ничего — SnackBar «Не нашлось приложения для карт».
- `mapUris(...)` вынесена top-level и покрыта тестами: она форматирует координаты через `toStringAsFixed`, а не `toString`, иначе на русской локали получилось бы `geo:55,76` и ссылка сломалась бы.
