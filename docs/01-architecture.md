...
graph TD
    ...
    subgraph text["Тексты — lib/l10n/"]
        L10n["Словарь<br/>ARB → AppL10n"]
    end

    subgraph data["Данные и платформа"]
        Yinmik["yinmik/<br/>BLE-протокол:<br/>декодер · клиент · команды"]
        History["history/<br/>drift: замеры, каталог мест,<br/>миграции, группировка"]
...
3. **Домен не знает про Flutter-виджеты.** Из UI-мира `quality/` использует только `Color` из `dart:ui` и словарь `AppL10n`. Словарь — не виджет: он строится из локали, и в тестах берётся одной строкой `lookupAppL10n(const Locale('ru'))`. Граница, за которой начинается тестируемость без `WidgetTester`, на месте.

   Отсюда же следует, что текст в домен **передаётся**, а не хранится: `WaterParameterCatalog.forProfile(profile, l10n)` вместо константного каталога. Границы зон и пороги шума одинаковы для всех языков, названия — нет.
...
5. **UI не ходит в БД напрямую.** Между ними `HistoryRepository` / `PlaceCatalogRepository`, принимающие доменные типы. Замена drift на что-то другое — это переписывание `repository.dart`, а не экранов.
...
sequenceDiagram
    ...
    R->>C: readOnce(device)
    R->>L: координаты, если разрешение уже есть
    L-->>R: точка или ничего
    R->>R: matchSite → место и его источник
    R->>H: latestForPlace(прибор, адрес)
    H-->>R: прошлый замер здесь или null
    ...
    R-->>U: карточки параметров с дельтами
...
Три свойства этого потока заданы намеренно и не должны потеряться при доработках:
...
- **Адрес замера хранится именами трёх уровней, а не ссылкой на каталог.** Место, комната и источник записываются в сам замер, поэтому переименование или удаление в каталоге не переписывает историю. Колонка `label` при этом означает источник — если бы в неё писался весь путь, обновление разорвало бы тренды.
- **Автоопределение места ничего не требует от пользователя.** Разрешение оно не запрашивает, ручной выбор не перебивает, а при неоднозначности не подставляет ничего. Ошибка подстановки дороже её отсутствия: замер уходит в историю не туда, и человек этого не видит.
- **База сравнения читается параллельно с прибором и не блокирует показания.** Запрос истории идёт своей дорогой: он нужен только для дельт, и его сбой оставляет экран без сравнения, но не без замера. По той же причине база берётся по выбранному месту — сравнение крана с бассейном показывало бы скачок качества там, где сменили точку забора.
...
- `intl` ^0.20.2 + `flutter_localizations` — локализация. Язык выбирается в настройках, по умолчанию определяется по системе; подробности — в [`05-state-and-storage.md`](./05-state-and-storage.md#локализация).
